# Multi-Task / Multi-Thread / Parallel Processing Architecture
...
### 4.1.1 CUDA partitioning for PDSCH DMRS / CSI-RS prep (code-level)

Key kernels and launch shapes live in `cuPHY/src/cuphy/pdsch_dmrs/pdsch_dmrs.cu`:

- **DMRS generation** (`fused_dmrs`):
  - `gridDimY = num_TBs` (one Y-slice per TB); `gridDimX = re_blocks_x` (RE blocks along frequency/time).
  - `blockDimX = threads` (configured in launch config; typical power-of-two), `blockDimY/Z = 1`.
  - Descriptor (`pdschDmrsDescr_t`) copied H2D via `cudaMemcpyAsync` on the provided stream before launch.
  - Kernel args (`m_kernelArgs[0]`) point to the GPU descriptor; PTX function pointer stored in `m_kernelNodeParams.func`.

- **PDSCH+CSI-RS prep** (`pdschCsirsPrepLaunchConfig` has three kernels):
  - Kernel 1 (`csirs_kernel1_device_function`): `blockDimX = block_size`, `gridDimX = (total_offsets + block_size - 1) / block_size` to sweep RE offsets; used to zero/init buffers.
  - Kernel 2 (`csirs_kernel2_device_function`): `blockDimX = 32 * OFDM_SYMBOLS_PER_SLOT`, `gridDimX = num_ue_groups` to cover UE-group/symbol combinations for remap.
  - Kernel 3 (`csirs_kernel3_device_function`): `blockDimX = num_threads`, `gridDimX = blocks` for post-processing/remap passes.
  - All kernels share the same descriptor pointer (copied async), with `kernelParams` populated per node.

- **CRC buffer prep** (`crc/prepareCrcBuffersTest.cu`):
  - Uses `PdschPerTbParams` to compute TB offsets/padding; copies parameter array H2D (`cudaMemcpyAsync`).
  - TB partitioning is performed per-TB with padded lengths (`cumulativeTbSizePadding`) to align CRC input; kernels (not shown) consume per-TB ranges.

These launch shapes show how work is partitioned:
- Y-dimension for TB-parallelism; X-dimension for RE or offset tiling. With `gridDimY=num_TBs`, each SM can pick up different TBs; warps within a block sweep adjacent REs, keeping memory coalesced.
- BlockDimX sized for coalescing/occupancy; no shared memory in DMRS (`sharedMemBytes = 0`), relying on L1/texture cache for RE lookups and regular striding to avoid bank conflicts.
- Descriptor copies are asynchronous and stream-scoped, enabling overlap across slots when multiple streams are used.
- In practice, SMs are fed by (gridX * gridY) blocks. For small TB counts, `gridDimY` keeps SMs busy as RE spans (`gridDimX`) grow; for large TB counts, RE-parallelism shifts to filling warp lanes within each block, leaving SM residency limited by register count and blockDimX. Occupancy tuning happens via `threads` (BlockDimX) in the launch config.
